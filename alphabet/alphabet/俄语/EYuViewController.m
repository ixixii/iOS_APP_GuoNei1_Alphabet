...
//  alphabet
//
//  Created by beyond on 2020/03/17.
//  Copyright © 2020 Christine. All rights reserved.
//

#import "EYuViewController.h"
...
@implementation EYuViewController
...
- (void)loadDataFromPlist
{
    ...
    NSString *fullPath = [mainBundle pathForResource:@"k_eyu_Alpha_List" ofType:@"plist"];
...
//        _alphaModelArr = [RiYuAlphaModel objectArrayWithKeyValuesArray:classDictArr];
    
        _alphaModelArr = [EYuAlphaModel objectArrayWithKeyValuesArray:classDictArr];
}
...
- (void)addBgImageView
{
    ...
    bgImageView.image = [UIImage imageNamed:@"IMG_4398.jpg"];
...
//    if (action == @selector(showItemClicked) ) {
//        return NO;
//    }
...
{
//    NSLog(@"sg__%@__%f",titleString,fontSize);
...
- (void)customBtnClicked:(UIButton *)btn
{
    ...
    _currentIndex = index;
    // asdf 这儿有问题
    EYuAlphaModel *model_tmp = [_alphaModelArr objectAtIndex:index];
    ...
    if ([preModel.alpha_remark containsString:@"难"]) {
        //        dispatch_async(dispatch_get_main_queue(), ^{
        //            [_preBtn setTitleColor:kHardAlphaColor forState:UIControlStateNormal];
        //        });
...
{
    NSString *fullPath = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@",model.alpha_mp3] ofType:nil];
    // 播放本地的音频
